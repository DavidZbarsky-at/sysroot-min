...
mv "./usr/lib/$LIB_ARCH-linux-gnu-backup"/{*.o,libc*,libdl*,libm*,libpthread*,libresolv*,librt*,libutil*} "./usr/lib/$LIB_ARCH-linux-gnu"
rm -rf "./usr/lib/$LIB_ARCH-linux-gnu-backup"

if [ -n "$EXTRA_PATH" ]; then
  tar -cJf "$NAME.tar.xz" lib/ usr/ "$EXTRA_PATH"
  tar -cf lib/ usr/ "$EXTRA_PATH" | zstd -o "$NAME.tar.zst"
else
  tar -cJf "$NAME.tar.xz" lib/ usr/
  tar -cf lib/ usr/ | zstd -o "$NAME.tar.zst"
fi
